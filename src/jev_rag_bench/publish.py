...
def render_text_tables(summaries: list[dict]) -> str:
    generated = datetime.now(UTC).strftime("%Y-%m-%d %H:%M UTC")
    lines = [f"Jev RAG Benchmark - real runs - generated {generated}", ""]
...
def render_dataset_card(summaries: list[dict], repo_url: str | None = None) -> str:
    datasets = ", ".join(summary.get("dataset", "unknown") for summary in summaries)
    lines = [
        "---",
        "license: mit",
    ...
    rows = [row for summary in summaries for row in rerank_rows(summary)]
    if rows:
        lines.append("## Reranking (nDCG@10, identical frozen top-20 candidates)")
        lines.append("")
        lines.append(
            _markdown_table(
                ["Dataset", "n", "Method", "nDCG@10", "Recall@5", "MRR@10", "Rerank p50"],
...
def push_to_huggingface(
    ...
    api = HfApi()
    create_kwargs: dict = {"repo_id": repo_id, "repo_type": repo_type, "exist_ok": True}
    if repo_type == "space":
